...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : NetworkBehaviour
    {
        ...
        [Space(10)]
        [Header("Fly Hack")]
        [Tooltip("Whether the fly toggle is available at all.")]
        public bool FlyHackEnabled = true;
        [Tooltip("Key that toggles fly mode on/off.")]
        public KeyCode FlyToggleKey = KeyCode.F7;
        [Tooltip("Base flight speed. Hold Sprint to go faster.")]
        public float FlySpeed = 8f;

        // true while this (local, owned) player is in fly mode
        ...
        private void Update()
        {
            ...
            if (FlyHackEnabled && Input.GetKeyDown(FlyToggleKey))
            {
                _flying = !_flying;
                // avoid a sudden fall or launch when toggling out of/into fly mode
        ...
        {
            // free-fly: move relative to the camera on all three axes, ignoring gravity/ground entirely
            Vector3 direction = _mainCamera.transform.forward * _input.move.y +
            ...
            float speed = _input.sprint ? FlySpeed * 2.5f : FlySpeed;
